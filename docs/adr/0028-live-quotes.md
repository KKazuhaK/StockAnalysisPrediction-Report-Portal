...
has already paid for: the sibling gap-fix line (the branch that also carries ADR 0027) spent a whole
release, v0.4.45 — "settings that were read and never written" — reconnecting six settings that had a
reader, a default and a comment describing a choice an operator could make, with nothing anywhere able
to make it. Its `internal/app/wired_settings_test.go` — on `claude/portal-gap-fixes-round1`, not in
this tree; `git ls-files` here will not find it — exists to stop it recurring. Adding a
