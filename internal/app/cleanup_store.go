...
func (s *Store) reportUIDsIngestedBefore(cutoff time.Time) ([]string, error) {
...
func (s *Store) CountReportsIngestedBefore(cutoff time.Time) (int64, error) {
	uids, err := s.reportUIDsIngestedBefore(cutoff)
	return int64(len(uids)), err
}

// DeleteReportsIngestedBefore deletes reports whose sent_at is a parseable RFC3339 instant older
// than cutoff, cascading their tracking_items in one transaction (children-first, mirroring
// DeleteReport). Returns how many reports were deleted.
