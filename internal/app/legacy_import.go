...
func RunLegacyImport(cfgPath string, logf func(string, ...any)) (imported, failed int, failedIDs []int64, err error) {
	...
	if base == "" {
		return 0, 0, nil, fmt.Errorf("old_base not configured — set the legacy portal URL/credentials under Settings first")
	}
	oc := NewOldClient(base, st.GetSetting("old_user", ""), st.GetSetting("old_pass", ""))
	im := &legacy.Importer{Src: legacySource{c: oc}, Sink: legacySink{s: st}, Log: logf}
	res, err := im.Run()
	return res.Imported, res.Failed, res.FailedIDs, err
}
