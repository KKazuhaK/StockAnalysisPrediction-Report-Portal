...
- **网页管理**（管理员）：入口按钮、报告类型（按大类分组/排序/默认页/改名/增删）、账号（角色）、系统设置（旧门户凭据/同步间隔）。
- **账号与角色**：可扩展的角色注册表（admin/user，易加更多）；首次启动自动创建 admin 并把密码打印到终端。
- **主题**：浅色/深色/跟随系统。**i18n**：多语言接口已预留（中文，英文可批量补）。
- **图标**：内嵌 Tabler(MIT) SVG 文件目录，加图标只需丢一个 `.svg`。
...
- **图标**：把 Tabler SVG 丢进 `icons/<name>.svg`，模板 `{{icon "name"}}` 即用。
...
config.go      YAML 基础设施配置
icons/         Tabler SVG 图标(内嵌)
templates/ static/   页面/样式/flatpickr(go:embed 进二进制)
```
